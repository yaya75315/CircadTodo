...
const DateHour = ({ day, date, hour, contentStyle }) => {
  ...
  const dateStyle =
    contentStyle === "dataChoose" ? "dateChooseStyle" : "dateNormal";
  const dotStyle = contentStyle === "dataChoose" ? "dotChooseStyle" : "dot";
  const hourStyle =
    contentStyle === "dataChoose" ? { color: "#000" } : { display: "block" };
  ...
  return (
    <div className={contentStyle}>
      <div className="dateContent">
        ...
        <p className={hourNumTag} style={hourStyle}>
          {hour}
        </p>
